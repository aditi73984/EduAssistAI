...
export default function CreatorDashboard() {
  ...
  const creatorRole = localStorage.getItem("creatorRole");
  const subjectSpecialist = localStorage.getItem("subjectSpecialist");
  const role = localStorage.getItem("role");

  if (role !== "creator") {
    return <h2>You are not a creator ❌</h2>;
  }
...
const creatorId = localStorage.getItem("creatorId") || "defaultCreator";
...
const uploadCourse = () => {
  const uniqueId = `creator-${Date.now()}`; // unique & stable id
  const enrolledKey = `enrolledCourses_${creatorId}`;
...
if (pdfFile) {
  ...
  // const old = JSON.parse(localStorage.getItem("creatorCourses") || "[]");
  const old = JSON.parse(
  localStorage.getItem(`creatorCourses_${creatorId}`) || "[]"
);

  old.push(newData);

  //localStorage.setItem("creatorCourses", JSON.stringify(old));
  localStorage.setItem(
  `creatorCourses_${creatorId}`,
  JSON.stringify(old)
);
      ...
      >
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            className="px-3 py-1 bg-emerald-600 text-white rounded shadow-sm"
          >
            + Add Course
          </motion.button>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col leading-tight text-right">
                ...
                <span className="text-xs text-gray-500">
                  {localStorage.getItem("creatorRole") || "Creator"}
                </span>
              </div>
            </div>
          </div>
...
{showAddModal && (
  <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div className="bg-white p-6 rounded-xl shadow-lg w-96">
      ...
      <h2 className="text-xl font-bold mb-4 text-green-950 text-center">Add New Course</h2>

      <input
        type="text"
        placeholder="Course Title"
        className="w-full p-2 border rounded mb-3 text-green-950"
        onChange={(e) => setNewCourse({ ...newCourse, title: e.target.value })}
      />

      <input
        type="text"
        placeholder="Color (purple / green / blue)"
        className="w-full p-2 border rounded mb-3 text-green-950"
      ...
      {/* <input
        type="text"
        placeholder="Course Link (ex: /Course/Python-1)"
        className="w-full p-2 border rounded mb-3 text-green-950"
...
/>
      ...
      <div className="flex justify-end gap-3">
        <button
          className="px-4 py-2 bg-gray-400 rounded text-green-950"
